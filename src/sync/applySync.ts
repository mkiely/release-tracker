...
export function applySync(
  ...
  for (const m of mapped.workStreams) {
    ...
    if (existing) {
      ...
      const ws: WorkStream = { id: uid('ws'), name: m.fields.name, externalId: m.externalId, engineersRequired: null, planningState: 'open', build: m.fields.build ?? null, externalUrl: m.fields.url ?? null, attributes: m.attributes ?? {} };
      workStreams.push(ws);
